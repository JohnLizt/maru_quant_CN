...
- Query API entrypoint: `docker compose exec app python api/cli/query_factors.py --symbol 603019.SH --date 2026-04-30`.
...
- Query entrypoint for external callers is `api/cli/query_factors.py`; it is a CLI-shaped API over the factor query service and should stay separate from internal ETL / factor pipeline scripts.
